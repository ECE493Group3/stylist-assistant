...
describe("User Wardrobe Input", function () {
    ...
    beforeEach(function () {
        ...
        browser.get('http://localhost:' + PORT + '/#/user_signin');
        
        username.sendKeys("test_user@capstone.ca");
        password.sendKeys("capstone");

        element(by.buttonText('Sign-In')).click();

        browser.get('http://localhost:' + PORT + '/#/user_upload_images/user/JJmxTJiDAAdKkehR7JucOOJb4OZ2');
    });
    ...
    it("User deletes photo on database", function (done) {
        ...
        browser.executeScript("document.querySelector('ion-content').scrollTop = document.querySelector('ion-content').scrollHeight")
            .then(function() {
                ...
                let deletebutton = $$('.item.editimages:first-child .delete');
                deletebutton.click();
